...
export default function Intro() {
  ...
  const renderItem = ({ item }: SlideItemsInterface) => {
    return (
      <VStack safeArea bg={'white'} flex={1} alignItems={'center'} pt={'8'}>
        <Text fontFamily={Righteous_400Regular} fontSize={'4xl'} color="primary.400" >{item.title}</Text>
        <View w="full" display={'flex'} >
          <Image source={item.background} size={96} resizeMode='contain' alt={item.alt_background} />
        </View>
        <View mx={6} mt={5}>
          <Text textAlign={'justify'} fontSize={'md'} fontFamily={Righteous_400Regular}>{item.text}</Text>
        </View>
      </VStack>
    )
  }
  ...
  const buttonNext = () => {
    return (
      <HStack backgroundColor={'primary.900'} width={'10'} height={'10'} alignItems={'center'} justifyContent={'center'} rounded={'full'}>
  ...
  const buttonSkip = () => {
    return (
      <HStack width={'20'} height={'8'} alignItems={'center'} justifyContent={'center'} rounded={'md'}>
