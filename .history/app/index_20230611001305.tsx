import { Button, FormControl, HStack, Stack, Text, VStack, useToast } from 'native-base';
...
export default function InitialScreen() {
  ...
  const login =  async (formData: any) => {
    ...
    if (error) {
      return toast.show({ title: error.response?.data?.message, backgroundColor: 'red.500', duration: 800 })
    ...
    <>
      <VStack alignItems={'center'} flex={1} justifyContent='center'>
        ...
        <Stack w="full" px={'6'} mt={'2'}>
          ...
          {isRegister ? (
            <Button mt={'4'} onPress={handleSubmit(register)}>Register</Button>
          ) : (
          ...
          {isRegister ? (
            <HStack justifyContent={'center'} space="2" mt={'2'}>
              ...
              <Text color={'#5ce1e6'} fontWeight={'900'} onPress={toggleRegister}>Login now</Text>
            </HStack>
          ) : (
            <HStack justifyContent='center' space="2" mt={'2'}>
